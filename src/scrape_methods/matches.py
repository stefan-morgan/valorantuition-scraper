...
class Matches:
    ...
    def match(self, id) :
        ...
        # My edit: Scrape the patch number
        patchString = soup.find_all('div', class_='match-header-date')[0].get_text().strip()
        startIndex = patchString.find("Patch")
        patch = patchString[startIndex:]
        patch = patch[:11]
        ...
        for map in mapStats:
            ...
            mapDuration = None
            if id != 'all':
                mapDuration = map.find_all("div", class_="map-duration")[0].get_text().strip()
            ...
            if id != 'all':
                ...
                for round in maprounds:
                    ...
                    try:
                        rounds.append({ 'roundNum': roundNum, 'roundScore': roundScore, 'winner': roundWinner, 'winType': winType })
            ...
            for row in scoreboard:
                for team in row.find_all('tr'):
                    ...
                    rating = team.find_all('td', class_='mod-stat')[0].find_all('span', class_='mod-both')[0].get_text().strip() # My addition
                    acs = team.find_all('td', class_='mod-stat')[1].find_all('span', class_='mod-both')[0].get_text().strip()
                    
                    kills = team.find_all('td', class_='mod-vlr-kills')[0].find_all('span', class_='stats-sq')[0].get_text().strip()
                    deaths = team.find_all('td', class_='mod-vlr-deaths')[0].find_all('span', class_='stats-sq')[0].get_text().strip().replace('/', '').strip()
                    assists = team.find_all('td', class_='mod-vlr-assists')[0].find_all('span', class_='stats-sq')[0].get_text().strip()
                    kast = team.find_all('td', class_='mod-stat')[6].find_all('span', class_='stats-sq')[0].get_text().strip()
                    ...
                    adr = team.find_all('td', class_='mod-stat')[7].find_all('span', class_='stats-sq')[0].get_text().strip()
                    hsperc = team.find_all('td', class_='mod-stat')[8].find_all('span', class_='stats-sq')[0].get_text().strip()
                    fk = team.find_all('td', class_='mod-fb')[0].find_all('span', class_='stats-sq')[0].get_text().strip()
                ...
                #print('')
            # My edit: stored map-id and map-duration
            mapData.append({'map-id': id, 'map': mapName, 'map-duration': mapDuration, 'teams': [team1Obj, team2Obj], 'members': members, 'rounds': rounds})
        ...
        bestOf = matchHeader.find_all("div", class_="match-header-vs-note")[1].get_text().strip()
        print(bestOf)
        
        return {'teams': teams, "best-of": bestOf, 'score': score, 'note': note, 'patch': patch, 'event': event, 'data': mapData, 'head2head': h2hMatches }
